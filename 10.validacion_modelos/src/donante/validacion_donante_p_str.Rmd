...
Genero valores NA's para un grupo de cada violencia y dejo los otros 4 grupos como parte del entrenamiento. Además hago algunos conteos de clase antes de la imputación para luego usarlos en el cálculo del WAI y WRAI: 
...
conteos_p_str_ant1 <- violencias_artificial_nas_1 %>%
	mutate(categoria = p_str) %>%
	group_by(categoria, violencia) %>%
	summarise(n_class= n()) %>%
	mutate(donante = "p_str_donante1") %>%
	ungroup() %>%
	group_by(violencia) %>%
...
#conteos de clase antes de la imputación
conteos_p_str_ant2 <- violencias_artificial_nas_2 %>%
	mutate(categoria = p_str) %>%
	group_by(categoria, violencia) %>%
	summarise(n_class= n()) %>%
	mutate(donante = "p_str_donante2") %>%
	ungroup() %>%
	group_by(violencia) %>%
	mutate(N = sum(n_class))
...
#conteos de clase antes de la imputación
conteos_p_str_ant3 <- violencias_artificial_nas_3 %>%
	mutate(categoria = p_str) %>%
	group_by(categoria, violencia) %>%
	summarise(n_class= n()) %>%
	mutate(donante = "p_str_donante3") %>%
	ungroup() %>%
	group_by(violencia) %>%
	mutate(N = sum(n_class))
...
#conteos de clase antes de la imputación
conteos_p_str_ant4 <- violencias_artificial_nas_4 %>%
	mutate(categoria = p_str) %>%
	group_by(categoria, violencia) %>%
	summarise(n_class= n()) %>%
	mutate(donante = "p_str_donante4") %>%
	ungroup() %>%
	group_by(violencia) %>%
	mutate(N = sum(n_class))
...
#conteos de clase antes de la imputación
conteos_p_str_ant5 <- violencias_artificial_nas_5 %>%
	mutate(categoria = p_str) %>%
	group_by(categoria, violencia) %>%
	summarise(n_class= n()) %>%
	mutate(donante = "p_str_donante5") %>%
	ungroup() %>%
	group_by(violencia) %>%
	mutate(N = sum(n_class))
...
comparison_data <- function(data) {
  results_list <- list()
  categorias_p_str <- unique(data$p_str_obs)
  for (categoria in categorias_p_str) {
    data_filtrada <- data[p_str_obs == categoria]
    
    for (i in 1:5) {
      donor_col <- paste0("p_str_donante", i)
      
      result <- data_filtrada[, .(
        iguales = sum(p_str_obs == get(donor_col), na.rm = TRUE),
        diferentes = sum(p_str_obs != get(donor_col), na.rm = TRUE),
        donante = donor_col 
      )]
...
generate_indicators_summary_by_class <- function(data) {
  ...
  # Inicializar una lista para almacenar los resúmenes por p_str
  summary_list <- list()
  
  # Agrupar por p_str
  p_strs <- unique(data_dt$categoria)
  
  # Iterar sobre cada p_str
  for (p_str in p_strs) {
    # Filtrar los datos correspondientes a la p_str actual
    p_str_data <- data_dt[categoria == p_str]
    
    # Calcular el resumen estadístico para cada métrica dentro de esta p_str
    stats_summary <- data.table(
      p_str = p_str,
      Metric = c("wai", "wrai"),
      Min = c(
        paste0(round(min(p_str_data$wai, na.rm = TRUE) * 100, 2), "%"),
        paste0(round(min(p_str_data$wrai, na.rm = TRUE) * 100, 2), "%")
      ),
      `1st_Qu.` = c(
        paste0(round(quantile(p_str_data$wai, 0.25, na.rm = TRUE) * 100, 2), "%"),
        paste0(round(quantile(p_str_data$wrai, 0.25, na.rm = TRUE) * 100, 2), "%")
      ),
      Median = c(
        paste0(round(median(p_str_data$wai, na.rm = TRUE) * 100, 2), "%"),
        paste0(round(median(p_str_data$wrai, na.rm = TRUE) * 100, 2), "%")
      ),
      Mean = c(
        paste0(round(mean(p_str_data$wai, na.rm = TRUE) * 100, 2), "%"),
        paste0(round(mean(p_str_data$wrai, na.rm = TRUE) * 100, 2), "%")
      ),
      `3rd_Qu.` = c(
        paste0(round(quantile(p_str_data$wai, 0.75, na.rm = TRUE) * 100, 2), "%"),
        paste0(round(quantile(p_str_data$wrai, 0.75, na.rm = TRUE) * 100, 2), "%")
      ),
      Max = c(
        paste0(round(max(p_str_data$wai, na.rm = TRUE) * 100, 2), "%"),
        paste0(round(max(p_str_data$wrai, na.rm = TRUE) * 100, 2), "%")
      ),
      SD = c(
        paste0(round(sd(p_str_data$wai, na.rm = TRUE) * 100, 2), "%"),
        paste0(round(sd(p_str_data$wrai, na.rm = TRUE) * 100, 2), "%")
      )
    )
    
    # Agregar el resumen estadístico de esta p_str a la lista
    summary_list[[p_str]] <- stats_summary
  }
...
comparison_desap_stat1 <- comparison_data(comparacion_desap) %>%
	filter(donante == "p_str_donante1") %>%
	mutate(violencia = "desaparicion") %>%
	left_join(conteos_p_str_ant1) 

comparison_desap_stat2 <- comparison_data(comparacion_desap) %>%
	filter(donante == "p_str_donante2") %>%
	mutate(violencia = "desaparicion") %>%
	left_join(conteos_p_str_ant2) 

comparison_desap_stat3 <- comparison_data(comparacion_desap) %>%
	filter(donante == "p_str_donante3") %>%
	mutate(violencia = "desaparicion") %>%
	left_join(conteos_p_str_ant3) 

comparison_desap_stat4 <- comparison_data(comparacion_desap) %>%
	filter(donante == "p_str_donante4") %>%
	mutate(violencia = "desaparicion") %>%
	left_join(conteos_p_str_ant4) 

comparison_desap_stat5 <- comparison_data(comparacion_desap) %>%
	filter(donante == "p_str_donante5") %>%
	mutate(violencia = "desaparicion") %>%
	left_join(conteos_p_str_ant5) 
