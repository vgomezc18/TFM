...
generate_indicators_summary_by_class <- function(data) {
  ...
  for (p_str in p_strs) {
    ...
    stats_summary <- data.table(
      ...
      Min = c(
        round(min(p_str_data$diferentes, na.rm = TRUE), 0),
        paste0(round(min(p_str_data$wrai, na.rm = TRUE) * 100, 2), "%")
      ),
      `1st_Qu.` = c(
        round(quantile(p_str_data$diferentes, 0.25, na.rm = TRUE), 0),
        paste0(round(quantile(p_str_data$wrai, 0.25, na.rm = TRUE) * 100, 2), "%")
      ),
      Median = c(
        round(median(p_str_data$diferentes, na.rm = TRUE), 0),
        paste0(round(median(p_str_data$wrai, na.rm = TRUE) * 100, 2), "%")
      ),
      Mean = c(
        round(mean(p_str_data$diferentes, na.rm = TRUE), 0),
        paste0(round(mean(p_str_data$wrai, na.rm = TRUE) * 100, 2), "%")
      ),
      `3rd_Qu.` = c(
        round(quantile(p_str_data$diferentes, 0.75, na.rm = TRUE), 0),
        paste0(round(quantile(p_str_data$wrai, 0.75, na.rm = TRUE) * 100, 2), "%")
      ),
      Max = c(
        round(max(p_str_data$diferentes, na.rm = TRUE), 0),
        paste0(round(max(p_str_data$wrai, na.rm = TRUE) * 100, 2), "%")
      ),
      SD = c(
        round(sd(p_str_data$diferentes, na.rm = TRUE), 2),
