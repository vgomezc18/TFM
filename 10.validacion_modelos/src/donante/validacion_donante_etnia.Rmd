...
conteos_etnia_ant1 <- violencias_artificial_nas_1 %>%
	mutate(categoria = etnia) %>%
	group_by(categoria, violencia) %>%
	summarise(n_class= n()) %>%
	mutate(donante = "etnia_donante1") %>%
	ungroup() %>%
	group_by(violencia) %>%
...
#conteos de clase antes de la imputación
conteos_etnia_ant2 <- violencias_artificial_nas_2 %>%
	mutate(categoria = etnia) %>%
	group_by(categoria, violencia) %>%
	summarise(n_class= n()) %>%
	mutate(donante = "etnia_donante2") %>%
	ungroup() %>%
	group_by(violencia) %>%
	mutate(N = sum(n_class))
...
#conteos de clase antes de la imputación
conteos_etnia_ant3 <- violencias_artificial_nas_3 %>%
	mutate(categoria = etnia) %>%
	group_by(categoria, violencia) %>%
	summarise(n_class= n()) %>%
	mutate(donante = "etnia_donante3") %>%
	ungroup() %>%
	group_by(violencia) %>%
	mutate(N = sum(n_class))
...
#conteos de clase antes de la imputación
conteos_etnia_ant4 <- violencias_artificial_nas_4 %>%
	mutate(categoria = etnia) %>%
	group_by(categoria, violencia) %>%
	summarise(n_class= n()) %>%
	mutate(donante = "etnia_donante4") %>%
	ungroup() %>%
	group_by(violencia) %>%
	mutate(N = sum(n_class))
...
#conteos de clase antes de la imputación
conteos_etnia_ant5 <- violencias_artificial_nas_5 %>%
	mutate(categoria = etnia) %>%
	group_by(categoria, violencia) %>%
	summarise(n_class= n()) %>%
	mutate(donante = "etnia_donante5") %>%
	ungroup() %>%
	group_by(violencia) %>%
	mutate(N = sum(n_class))
...
generate_indicators_summary_by_class <- function(data) {
  ...
  for (etnia in etnias) {
    ...
    stats_summary <- data.table(
      ...
      Metric = c("wai", "wrai"),
      Min = c(
        paste0(round(min(etnia_data$wai, na.rm = TRUE) * 100, 2), "%"),
        paste0(round(min(etnia_data$wrai, na.rm = TRUE) * 100, 2), "%")
      ),
      `1st_Qu.` = c(
        paste0(round(quantile(etnia_data$wai, 0.25, na.rm = TRUE) * 100, 2), "%"),
        paste0(round(quantile(etnia_data$wrai, 0.25, na.rm = TRUE) * 100, 2), "%")
      ),
      Median = c(
        paste0(round(median(etnia_data$wai, na.rm = TRUE) * 100, 2), "%"),
        paste0(round(median(etnia_data$wrai, na.rm = TRUE) * 100, 2), "%")
      ),
      Mean = c(
        paste0(round(mean(etnia_data$wai, na.rm = TRUE) * 100, 2), "%"),
        paste0(round(mean(etnia_data$wrai, na.rm = TRUE) * 100, 2), "%")
      ),
      `3rd_Qu.` = c(
        paste0(round(quantile(etnia_data$wai, 0.75, na.rm = TRUE) * 100, 2), "%"),
        paste0(round(quantile(etnia_data$wrai, 0.75, na.rm = TRUE) * 100, 2), "%")
      ),
      Max = c(
        paste0(round(max(etnia_data$wai, na.rm = TRUE) * 100, 2), "%"),
        paste0(round(max(etnia_data$wrai, na.rm = TRUE) * 100, 2), "%")
      ),
      SD = c(
        paste0(round(sd(etnia_data$wai, na.rm = TRUE) * 100, 2), "%"),
...
comparison_desap_stat1 <- comparison_data(comparacion_desap) %>%
	filter(donante == "etnia_donante1") %>%
	mutate(violencia = "desaparicion") %>%
	left_join(conteos_etnia_ant1) 

comparison_desap_stat2 <- comparison_data(comparacion_desap) %>%
	filter(donante == "etnia_donante2") %>%
	mutate(violencia = "desaparicion") %>%
	left_join(conteos_etnia_ant2) 

comparison_desap_stat3 <- comparison_data(comparacion_desap) %>%
	filter(donante == "etnia_donante3") %>%
	mutate(violencia = "desaparicion") %>%
	left_join(conteos_etnia_ant3) 

comparison_desap_stat4 <- comparison_data(comparacion_desap) %>%
	filter(donante == "etnia_donante4") %>%
	mutate(violencia = "desaparicion") %>%
	left_join(conteos_etnia_ant4) 

comparison_desap_stat5 <- comparison_data(comparacion_desap) %>%
	filter(donante == "etnia_donante5") %>%
	mutate(violencia = "desaparicion") %>%
	left_join(conteos_etnia_ant5) 
