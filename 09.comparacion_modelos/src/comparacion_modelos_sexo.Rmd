...
p_load(here, readr, tidyverse, dplyr, validate, data.table, missForest, 
			 VIM, lattice, simputation, mice, skimr, compareDF, verdata)
...
imputacion_ml_sexo <- readRDS(paste0(imputacion_dir,"/imputacion_ml_sexo_under_light.rds")) 
...
Inserto los resultados por LightGBM: 
...
plot_desap_mujeres_year_imp <- mujeres_proyecto_desap %>%
  ...
  geom_line(aes(y = imp_ml, color = "Imputado por LightGBM"), size = 1, linetype = "dotdash") +
...
plot_reclu_mujeres_year_imp <- mujeres_proyecto_reclu %>%
    ...
    theme(axis.text.x = element_text(size = 11, angle = 90),
        ...
        legend.box.margin = margin(0, 0, 10, 0)) +
    scale_x_continuous(breaks = seq(1980, 2016, 2)) +
		scale_y_continuous(breaks = seq(0, max(mujeres_proyecto_reclu$imp_ml,
																				 na.rm = TRUE), by = 100)) +
    theme(legend.position = "bottom") +
    labs(x = "Año",
         y = "Número de víctimas mujeres",
