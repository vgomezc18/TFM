...
```{r}
donante_sexo_desap <- rbind(imputacion_donante_sexo[["desaparicion_1996_2000"]], 
...
donante_sexo_homi <- rbind(imputacion_donante_sexo[["homicidio_1996_2000"]], 
...
donante_sexo_secue <- rbind(imputacion_donante_sexo[["secuestro_1996_2000"]], 
...
donante_sexo_reclu <- rbind(imputacion_donante_sexo[["reclutamiento_1996_2000"]], 
...
```{r}
regresion_sexo_desap <- as.data.frame(imputacion_regresion_sexo[["desaparicion_1996_2016"]])

regresion_sexo_homi <- as.data.frame(imputacion_regresion_sexo[["homicidio_1996_2016"]])

regresion_sexo_secue <- as.data.frame(imputacion_regresion_sexo[["secuestro_1996_2016"]])

regresion_sexo_reclu <- as.data.frame(imputacion_regresion_sexo[["reclutamiento_1996_2016"]])
```

Inserto los resultados por machine learning: 
...
plot_desap_mujeres_year_imp <- mujeres_proyecto_desap %>%
  ...
  geom_line(aes(y = imp_ml, color = "Imputado por Machine Learning"), size = 1, linetype = "dotted") +
  ...
  scale_colour_manual(values = c("Observado" = "#434343",
                                 ...
                                 "Imputado por Machine Learning" = "#FFC300")) +
...
plot_reclu_mujeres_year_imp <- mujeres_proyecto_reclu %>%
    ...
    geom_line(aes(y = imp_mean,  color = "Imputado JEP-CEV-HRDAG"), size = 1) +
	  ...
	  geom_line(aes(y = imp_regresion_log,  color = "Imputado por Regresión Log"),
	  					size = 1, linetype = "dashed") +
	  geom_line(aes(y = imp_ml, color = "Imputado por Machine Learning"), size = 1, linetype = "dotdash") +
	  geom_ribbon(aes(ymin = imp_lo,
                    ymax = imp_hi),
	  						fill = "#1F74B1",
                alpha = 0.2) +
